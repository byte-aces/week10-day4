    ...
    if request.method == 'GET':
        ...
        if username and password and password_confirm:
            ...
            if password == password_confirm:
                ...
                connection = sqlite3.connect('../datastore/sqlite.db', check_same_thread=False)
...
def login():
    ...
    if request.method == 'GET':
        ...
        if username and password:
            ...
            cursor     = connection.cursor()
            # --> Execute a query
            cursor.execute('SELECT COUNT(*), MIN(pk) FROM users WHERE username="{0}" AND password="{1}";'.format(username, password))
            (tally, customer_id) = cursor.fetchall()
            if tally < 1:
                # bad creds
                pass
            elif tally > 1:
